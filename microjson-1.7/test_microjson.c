...
/* Case 19: Check that t_check catches bad dflt.check values */

static int i19 = 19;
static char *json_str19 = "{\"class\":\"foo\",\"mode\":-4}";
static const struct json_attr_t json_attrs_19[] = {
    {"class", t_check, .dflt.check = "TPV"},
    {"mode", t_integer, .addr.integer = &i19, .dflt.integer = -9},
    {NULL},
};

/* Case 20: test the check feature and ignore */ 

static char revision[50];
static uint16_t pvhi, pvlo;
static char *json_str20 = "{\"class\":\"VERSION\",\"release\":\"3.19.1~dev\",\"rev\":\"release-3.19-"
    				"655-gb4aded4c1\",\"proto_major\":3,\"proto_minor\":14}";
const struct json_attr_t json_attrs_20[] = {
    {"class", t_check, .dflt.check = "VERSION"},
...
/* Case 21: Test multiple checks and ignore */

static int tally = 0;
static bool enable, json;
static char *json_str21 =
    "{\"class\":\"WATCH\",\"enable\":true,\"json\":true,\"nmea\":false,\"raw\":"
    "0,\"scaled\":false,\"timing\":false,\"split24\":false,\"pps\":false,"
    "\"device\":\"/dev/ttyUSB0\"}";
const struct json_attr_t json_attrs_21[] = {
...
/* Case 22:Another chwck and ignore */

static int gps_mode;
static double ept;
static char gps_time[50];
static char *json_str22 =
    ...
    "15.200,\"sep\":31.273}";
const struct json_attr_t json_attrs_22[] = {
    {"class", t_check, .dflt.check = "TPV"},
    {"device", t_check, .dflt.check = "/dev/ttyUSB0"},
...
static void jsontest(int i)
{
	...
	assert_case(i, status);
	assert_integer("proto_major", 3, pvhi);
	assert_integer("proto_minor", 14, pvlo);
	break;

    case 21:
	status = json_read_object(json_str21, json_attrs_21, NULL);
	assert_case(i, status);
	assert_boolean("enable", true, enable);
	assert_boolean("json", true, json);
	break;

    case 22:
	status = json_read_object(json_str22, json_attrs_22, NULL);
	assert_case(i, status);
	assert_integer("gps_mode", 3, gps_mode);
	assert_real("ept", 0.005, ept);
	assert_string("gps_time", "2019-10-04T08:51:34.000Z", gps_time);
	break;
